...
There is intentionally **no chat, AI, or network logic**. The chat window is a **MOCKUP** (dead-end: Send only replays a spark animation). A **scripted-demo chat is ΦΑΣΗ 2, pending** — hardcoded example answers on the existing hooks, still no live AI/network. Treat this repo as the visual source of truth; do not add fetch/API/state-management code unless explicitly asked.
...
- **Send button** toggles `.is-ready` (vermilion glow) when `#ask-input` holds text, and replays a `.spark` sweep on click; pressing Enter in the input replays the same spark (still no submit logic).
...
The inline `<script>` is intentionally minimal. These IDs/hooks exist for the backend (or the ΦΑΣΗ 2 scripted demo) to connect later — keep them in place:
